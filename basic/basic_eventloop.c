...
void custom_loop_init(void) {
    ...
    // For example:
    // subscribe(MODULE_NAME, 2, CONTROL_ONE, CONTROL_TWO)

    subscribe(MODULE_KEYBOARD, 1, CONTROL_TIMER_KEYBOARD_POLLING );
    subscribe(MODULE_APP,      3, CONTROL_KEYBOARD,
                                  CONTROL_TIMER_APP_DEMO,
                                  CONTROL_TIMER_ONE_SHOT_DEMO );


    // Set the soft timers
    // The parameters are CONTROL_TIMER_XXXX, time, timer type, enabled/disabled
    // For example:
    // add_timer_event(CONTROL_TIMER_XXXX, 300, TIMER_ONE_SHOT, TIMER_DISABLED)

    //CONTROL_TIMER_KEYBOARD_POLLING,    // 0.02 seconds
    //CONTROL_TIMER_APP_DEMO,            // 10 seconds
    //CONTROL_TIMER_ONE_SHOT_DEMO,       // 5 seconds

    
    add_timer_event(CONTROL_TIMER_APP_DEMO, 10000,  TIMER_REPEAT, TIMER_ENABLED);
    add_timer_event(CONTROL_TIMER_KEYBOARD_POLLING, 100,  TIMER_REPEAT, TIMER_ENABLED);
    add_timer_event(CONTROL_TIMER_ONE_SHOT_DEMO, 5000, TIMER_ONE_SHOT, TIMER_DISABLED);
...
void custom_loop_scheduler(unsigned int module_list, controls control, 
    ...
    //printf("custom_loop_scheduler control %d, value %i \n", control, value);

    if (module_list & MODULE_01_MASK) {
        // call module 01 here
        keyboard_module_control(control, value);
    }

    if (module_list & MODULE_02_MASK) {
        // call module 01 here
        app_module_control(control, value);
    }

    if (module_list & MODULE_03_MASK) {
        // call module 03 here
        // generic_module_call_control(control, value);
    }
